...
async def cmd_start(message: Message) -> None:
    ...
    # Сохраняем реферальный код в FSM data до создания пользователя
    args = message.text.split(maxsplit=1)
    if len(args) > 1 and args[1].startswith("ref_"):
        ref_code = args[1][4:]  # убираем "ref_"
        from api.auth.session import get_fsm_data, set_fsm_data
        data = await get_fsm_data(str(message.from_user.id)) or {}
        data["pending_ref_code"] = ref_code
        await set_fsm_data(str(message.from_user.id), data)
