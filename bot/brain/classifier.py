...
PATTERN_MAP: list[tuple[str, Intent, bool]] = [
    ...
    (r"помог|что умеешь|список команд|что ты умеешь|команды", Intent.HELP, True),
    ...
    (r"настройк|настроить бота", Intent.SETTINGS, True),

    # --- Профиль ---
    (r"мой профил|моя анкет|посмотреть профил", Intent.PROFILE_VIEW, True),
    (r"(измени|редактир|поменя|сменить|обнови).*(профил|имя|никнейм|ассистент)", Intent.PROFILE_EDIT, False),

    # --- Экономика ---
    (r"^баланс$|^мой баланс$|^мои монеты$|сколько.*(монет|экоин|ecoins)|покажи баланс", Intent.BALANCE, True),
    (r"^бонус$|ежедневн|бонус дня|дейли", Intent.DAILY_BONUS, True),
    (r"^(передать|дать|отдать|скинуть)\s+\d+", Intent.TRANSFER, True),
    (r"^рефералы$|^реф$|^рефер$|реферальная ссылка|моя реф", Intent.REFERRAL, True),
    (r"^лидеры$|^топ$|таблица лидеров|лучшие по монетам", Intent.LEADERBOARD, True),

    # --- Питомец ---
    (r"^питомец$|^мой питомец$|меню питомца|главное меню питомца", Intent.PET_MENU, True),
    ...
    (r"^чс$|^мой чс$|^чёрный список$|покажи чс|покажи чёрный список|список заблокированных|мои заблокированные", Intent.BLACKLIST_VIEW, True),

    # --- События ---
    (r"(создай|создать|новое|организ).*(событи|встреч|меропри)", Intent.EVENT_CREATE, True),
    (r"(список|ближайш|покажи).*(событи|встреч)", Intent.EVENT_LIST, True),
    (r"(участвовать|участвую|присоединить|хочу на).*(событи|встреч)", Intent.EVENT_JOIN, False),

    # --- Казино ---
    # казино — только без аргументов
    (r"^казино$", Intent.CASINO_OPEN, True),
    # слоты [ставка]
    (r"^слоты(\s+\d+)?$", Intent.CASINO_SLOTS, True),
    # рулетка [тип] [ставка]  — тип: к/ч/чет/нечет/мало/много/0-36
    (r"^рулетка(\s+\S+)?(\s+\d+)?$", Intent.CASINO_ROULETTE, True),
    # кости [число 1-6] [ставка]
    (r"^кости(\s+\d+)?(\s+\d+)?$", Intent.CASINO_DICE, True),
    # монетка [о/р] [ставка]
    (r"^монетка(\s+\S+)?(\s+\d+)?$", Intent.CASINO_COIN, True),
    # мины [ставка]
    (r"^мины(\s+\d+)?$", Intent.CASINO_MINES, True),
    # джокер [ставка]
    (r"^джокер(\s+\d+)?$", Intent.CASINO_JOKER, True),
    # колесо [ставка]
    (r"^колесо(\s+\d+)?$", Intent.CASINO_WHEEL, True),

    # --- Мини-игры ---
    (r"викторина|quiz|задай вопрос|тест на знания", Intent.GAME_QUIZ, True),
    (r"кубик|брось кубик|кинь кубик", Intent.GAME_DICE, False),  # /dice убран — занят CASINO_DICE
    (r"правда или действие|truth or dare", Intent.GAME_TRUTH_DARE, True),
    (r"что бы ты выбрал|что лучше|выбор между", Intent.GAME_WOULD_YOU, True),
    (r"загадк|загадай|задай загадку", Intent.GAME_RIDDLE, True),
    ...
    (r"(какая|какой|узнай|скажи|покажи|будет).*(погода|погоду|температур|осадки|дождь|снег|ветер)|(какой|какая).*(прогноз|погода)|прогноз на (завтра|неделю|сегодня)|прогноз погоды|погода в", Intent.WEATHER, True),
    (r"(как сказать|как будет|как переводится|переведи|перевод|переводи).*(на |по )|как по-(русски|английски|немецки|французски|испански|китайски|японски)|как сказать .* по|translate", Intent.TRANSLATE, True),
    ...
    (r"(найди|рекоменд|посоветуй|хочу читать|ищу).*(книг|автор|роман|повест)", Intent.BOOK_SEARCH, True),
    (r"(найди|рекоменд|посоветуй|хочу смотреть|ищу).*(аниме|anime)", Intent.ANIME_SEARCH, True),

    # --- Задачи ---
    (r"(создай|добавь|новая|запиши|поставь).*(задач|todo|дело|цель)", Intent.TASK_CREATE, True),
    (r"(мои|список|покажи|все).*(задач|todo|дела)", Intent.TASK_LIST, True),
    (r"(выполнил|сделал|готово|закрой|отметь).*(задач|дело|пункт)|задача выполнена", Intent.TASK_DONE, False),
    (r"(напомни|напоминание|установи напоминание|не забудь|remind)", Intent.REMINDER_CREATE, True),

    # --- AI чат — явный запрос на разговор ---
    (r"поговори со мной|давай поговорим|пообщайся|поболтай", Intent.AI_CHAT, True),

    # --- Модерация групп ---
    # Снять варн / варны — ВЫШЕ варна, чтобы не перехватывался
    (r"снять варн|убрать варн|удалить варн|\-варн|\bunwarn\b", Intent.GROUP_UNWARN, True),
    (r"\bварны\b|сколько варнов|проверить варны", Intent.GROUP_WARNS, True),
    (r"очистить варны|сбросить варны|снять варны|убрать варны", Intent.GROUP_CLEARWARNS, True),
    # Варн
    (r"\bварн\b|варнить|предупредить|предупреждение\b|\bwarn\b", Intent.GROUP_WARN, True),
    # Разбан — ВЫШЕ бана
    (r"\bразбан\b|разбанить|разблокировать\b|\bunban\b", Intent.GROUP_UNBAN, True),
    # Бан
    (r"\bбан\b|забанить|\bban\b", Intent.GROUP_BAN, True),
    # Размут — ВЫШЕ мута
    (r"\bразмут\b|размутить|разглушить|говори снова|\bunmute\b", Intent.GROUP_UNMUTE, True),
    # Мут
    (r"\bмут\b|замутить|заглушить|помолчи|заткнись|молчать|замолчи|\bsilence\b|\bmute\b", Intent.GROUP_MUTE, True),
    # Кик
    # Повышение / понижение
    (r"\bповысить\b|повышение\b|продвинуть\b|\bpromote\b|назначить\b", Intent.GROUP_PROMOTE, True),
    (r"\bпонизить\b|понижение\b|разжаловать\b|\bdemote\b|снять роль", Intent.GROUP_DEMOTE, True),
    # Настройки / статистика / приветствие
    (r"настройки группы", Intent.GROUP_SETTINGS, True),
    (r"\bстатистика\b|активность группы", Intent.GROUP_STATS, True),
    (r"приветствие группы|настроить приветствие", Intent.GROUP_WELCOME, True),
    # Роль / администраторы — ВЫШЕ профиля чтобы не перехватывался
    (r"моя роль|какая моя роль|кто я в группе|мой статус в группе|мои права в группе", Intent.GROUP_ROLE, True),
    (r"список админов|кто админ|администраторы группы|модераторы группы|кто модератор|кто управляет", Intent.GROUP_ADMINS, True),
]
