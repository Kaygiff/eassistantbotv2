...
from api.auth.session import get_fsm_state
...
async def handle_fsm(ctx: BrainContext, bot) -> bool:
    ...
    if state.startswith("onboarding:"):
        from bot.onboarding.flow import handle_onboarding_text
        return await handle_onboarding_text(ctx, bot)

    # --- Настройки профиля ---
    if state.startswith("settings:"):
        from bot.onboarding.profile_edit import handle_profile_edit_fsm
