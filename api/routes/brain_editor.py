...
@router.get("/stats")
async def brain_stats(_=require_admin):
    return await get_editor_stats()


@router.get("/rules")
async def list_rules(_=require_admin):
...
async def update_brain_rule(intent: str, body: RuleUpdate, _=require_admin):
...
async def delete_brain_rule(intent: str, _=require_admin):
    await delete_rule(intent)
    return {"ok": True}


@router.post("/rules/reload")
async def reload_rules(_=require_admin):
