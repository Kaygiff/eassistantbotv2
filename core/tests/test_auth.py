...
class TestSession:
    ...
    async def test_set_and_get_session(self, fake_redis):
        with patch("auth.session.get_redis", return_value=fake_redis):
            from api.auth.session import set_session, get_session
    ...
    async def test_get_empty_session(self, fake_redis):
        with patch("auth.session.get_redis", return_value=fake_redis):
            from api.auth.session import get_session
    ...
    async def test_update_session(self, fake_redis):
        with patch("auth.session.get_redis", return_value=fake_redis):
            from api.auth.session import set_session, update_session, get_session
    ...
    async def test_fsm_state(self, fake_redis):
        with patch("auth.session.get_redis", return_value=fake_redis):
            from api.auth.session import set_fsm_state, get_fsm_state, clear_fsm_state
