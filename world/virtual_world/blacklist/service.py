...
async def add_to_blacklist(blocker_id: str, blocked_username: str, language: str) -> str:
    res = get_supabase_admin().table("users").select("id, first_name").eq("username", blocked_username).maybe_single().execute()
    if not res.data:
    ...
    get_supabase_admin().table("blacklist").upsert({
        "id": str(uuid.uuid4()),
...
async def remove_from_blacklist(blocker_id: str, blocked_username: str, language: str) -> str:
    ...
    blocked_id = res.data["id"]
    get_supabase_admin().table("blacklist").delete().eq("blocker_id", blocker_id).eq("blocked_id", blocked_id).execute()
...
async def is_blocked(blocker_id: str, blocked_id: str) -> bool:
    res = get_supabase_admin().table("blacklist").select("id").eq("blocker_id", blocker_id).eq("blocked_id", blocked_id).maybe_single().execute()
